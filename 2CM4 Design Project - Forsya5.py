'''
ENGPHYS 2CM4 Design Project: 
     Optimizing a Security Strap on Piezoelectric Plasma Micro-Thrusters for Nanosatellites 
...
FlexCode = """TITLE 'PPMT strap optimization'    
...
warnings.filterwarnings("ignore") #to hide local warngings that are displayed in the result console

FlexFileName = "2CM4ProjectForsya5.pde"

import matplotlib.pyplot as plt


LyStraprange = sp.arange(1e-6, 7e-6, 1e-6)

minimumDisplacment = 1e3

for LyStrap in LyStraprange:
    with open(FlexFileName, "w") as f:
        print(FlexCode%LyStrap, file=f)
    subprocess.run(["FlexPDE7","-S",FlexFileName],timeout=10)#,shell=True)

    
    with open("2CM4ProjectForsya5_output/2CM4ProjectForsya5.txt", "r") as f:
        Line1 = f.readlines() #outputs the text file into seperate strigs, line by line
        Lines = Line1[-1] #only outputs the last line, which represents the Tip-Displacement
        Line = re.findall("[-+]?\d+[\.]?\d*[Ee]?\d*", Lines) #to extract the string from the text file into a single float
        Num = float(Line[0]+Line[1]) #combines the float list into 1 float

    displacement = Num
    
    print("{LyStrap:.1e}\t {displacement:.2e}".format(LyStrap = LyStrap, displacement=displacement))

    displacement1 = [displacement]
    
    yvalue = [-displacement, displacement]
    xvalue = [0,30]
    
    
'''    
    plt.plot(xvalue, yvalue, Label=LyStrap)
    
    if(abs(displacement1[-1]) < abs(minimumDisplacment)):
       minimumDisplacment  = abs(displacement1[-1])
       OptimalLyStrap = LyStrap
        
print("Smallest displacement of the PPMT was {displacement:.2e} [m], based off an aluminum strap height of {LyStrap:.2e} [m].".format(LyStrap = OptimalLyStrap, displacement=minimumDisplacment))


plt.title('Tip-Displacement for various LyStrap values')
plt.xlabel('Length of PPMT [x10^-6 m]')
plt.ylabel('Tip Displacement [m]')
plt.legend(LyStraprange, loc='best')
plt.show()
